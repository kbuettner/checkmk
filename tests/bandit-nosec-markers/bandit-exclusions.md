...
| `BNS:3d8f2a`    | `B104`         | Intentional binding to `0.0.0.0` as IPv4 fallback when IPv6 is unavailable. This is expected server behavior.                                                                        |
| `BNS:b7e3d1`    | `B104`         | Default SNMP target address when no IP is configured. Used as a command argument, not a socket bind.                                                                                 |
| `BNS:c94f5e`    | `B104`         | Test data parameter for IP matching tests. The string `0.0.0.0` is not used for binding.                                                                                             |
| `BNS:d61a82`    | `B104`         | Guard check that rejects `0.0.0.0` and `::` by raising `FetcherError`. The opposite of what B104 warns about.                                                                        |
