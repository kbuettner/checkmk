    ...
    [
        (
            ...
            [
                Result(
                    state=State.OK,
                    summary="Usage: 91.41% - 15.4 MiB of 16.9 MiB (Parent pod memory usage)",
                ),
                Metric("mem_used", 16162816.0),
                Result(state=State.OK, summary="Resident size: 8812.0 kB"),
                Result(state=State.OK, summary="Cache: 6160.0 kB"),
                Metric("mem_lnx_cached", 6307840.0),
                Result(state=State.OK, summary="Swap: 0.0 kB"),
                Metric("swap_used", 0.0),
            ],
        ),
    ],
)
def test_check_cadvisor_memory(
    string_table: StringTable, expected_results: Sequence[Result | Metric]
) -> None:
